"""
ratio_forward_product_grammars_v2_1_full.py

Ratio-forward-like product grammar implemented on top of contract_model_v2_1_full.

v2.1 changes:
- Coupon Swap form uses AmountBasedCouponCalculation.
- Ratio-forward coupon exchange is represented by RatioForwardCouponAmountRule,
  not by a coupon/rate rule.
- FX Option Package form remains per-period option components with explicit sold-side KI.
- WKO is Observation + BarrierCondition + EventLifecycleRule/Effect.
...
from contract_model_v2_1_full import *
...
def _schedule_dates(schedule_like: ScheduleRefLike) -> tuple[date, ...]:
    ...
    raise TypeError("ratio-forward v2.1 builders expect materialized DateListSchedule inputs")
...
def build_coupon_swap_ratio_forward_contract_v2(
    ...
    form_id: str = "FORM-V2-1-COUPON-SWAP-RATIO-FWD",
    ...
    if grammar.economic_terms.sold_option_knock_in is not None:
        ...
        sold_side_condition_id = "cond_sold_side_ki_for_coupon_determination"
        conditions.append(
            BarrierCondition(
                sold_side_condition_id,
                ki_rule_id,
                grammar.economic_terms.sold_option_knock_in.direction,
                grammar.economic_terms.sold_option_knock_in.barrier,
            )
        )
    ...
    determination_rules.extend([
        ...
        RatioForwardCouponAmountRule(
            ...
            bought_side_quantity_rule_id="q_call_amount_base",
            sold_side_quantity_rule_id="q_put_amount_base",
            sold_option_selector=grammar.economic_terms.sold_option_selector.value,
            sold_side_condition_id=sold_side_condition_id,
        ),
        RatioForwardCouponAmountRule(
            "amt_coupon_swap_receive",
            pair=pair,
            scheme=grammar.economic_terms.scheme.value,
    ...
    components.extend([
        CouponStreamLeg(
            ...
            calculation=AmountBasedCouponCalculation("amt_coupon_swap_pay"),
        ...
        CouponStreamLeg(
            ...
            reference=pair_ref,
            payment_schedule=grammar.schedule.payment_schedule,
            calculation=AmountBasedCouponCalculation("amt_coupon_swap_receive"),
    ...
    form = ContractFormV2(
        form_id=form_id,
        form_kind="COUPON_SWAP_RATIO_FORWARD_V2_1",
        ...
        tags={
            "grammar_kind": "COUPON_SWAP_RATIO_FORWARD_V2_1",
            "scheme": grammar.economic_terms.scheme.value,
            "same_economics_as": "FX_OPTION_PACKAGE_RATIO_FORWARD_V2_1",
...
def build_fx_option_package_ratio_forward_contract_v2(
    ...
    form_id: str = "FORM-V2-1-FX-OPTION-PACKAGE-RATIO-FWD",
) -> BuiltRatioForwardContractV2:
    grammar.validate()
    client_id, bank_id = _client_and_bank_ids(grammar.counterparties)
    specs = expand_period_specs(grammar.schedule, grammar.economic_terms)
    ...
    for spec in specs:
        ...
        if spec.sold_option_knock_in is not None:
            ...
            obs_rule_id = f"obs_period_{p}_sold_option_ki"
            cond_id = f"cond_period_{p}_sold_option_ki"
            ...
            lifecycle_rules.append(
                EventLifecycleRule(
                    lifecycle_rule_id=f"life_period_{p}_sold_option_ki",
                    trigger=Trigger(f"trigger_period_{p}_sold_option_ki", cond_id, DateListSchedule((obs_date,))),
                    effects=(ActivateComponentsEffect((sold_leg_id,)),),
                )
            )
    ...
    form = ContractFormV2(
        form_id=form_id,
        form_kind="FX_OPTION_PACKAGE_RATIO_FORWARD_V2_1",
        parties=grammar.counterparties.both(),
        party_roles=(PartyRoleAssignment("client", client_id), PartyRoleAssignment("bank", bank_id)),
        references=(pair_ref,),
        observables=tuple(observables),
        ...
        lifecycle_rules=tuple(lifecycle_rules),
        tags={
            "grammar_kind": "FX_OPTION_PACKAGE_RATIO_FORWARD_V2_1",
            "scheme": grammar.economic_terms.scheme.value,
            "same_economics_as": "COUPON_SWAP_RATIO_FORWARD_V2_1",
...
def build_example_coupon_swap_gap_wko_v2() -> BuiltRatioForwardContractV2:
    ...
    return build_coupon_swap_ratio_forward_contract_v2(grammar, "EXAMPLE-V2-1-COUPON-SWAP-GAP-WKO")
...
def build_example_fx_option_package_two_stage_target_v2() -> BuiltRatioForwardContractV2:
    ...
    return build_fx_option_package_ratio_forward_contract_v2(grammar, "EXAMPLE-V2-1-FX-OPTION-PACKAGE-TWO-STAGE-TARGET")
