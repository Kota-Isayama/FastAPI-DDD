...
Form-first derivative contract model v2.

This version intentionally moves away from a single catch-all `Formula` bucket.
Instead it models a financial contract as:

- Reference / Observable
- Event / Schedule
- Economic Action / Component
- Determination Rule
- Lifecycle / State Rule
- Overlay / Modifier
- Condition / Trigger / Effect

The goal is not to price every product.  The goal is to provide a clean,
declarative contract representation that can support authoring, editing,
normalization and simulation without losing the contract form.
"""
...
class ObservationStyle(str, Enum):
    EUROPEAN = "EUROPEAN"      # usually one designated observation date
    AMERICAN = "AMERICAN"      # any date in an observation window/schedule
    BERMUDAN = "BERMUDAN"      # discrete observation dates
    CONTINUOUS = "CONTINUOUS"  # conceptually continuous monitoring
...
class MtMResetQuantityRule(QuantityRule):
    """Determines a new quantity from an observed reference value."""
    rule_id: str
...
class CouponDeterminationRule(DeterminationRule):
    pass


@dataclass(frozen=True)
class RateCouponRule(CouponDeterminationRule):
    rule_id: str
    rate_rule_id: str


@dataclass(frozen=True)
class StructuredCouponPayoffRule(CouponDeterminationRule):
    rule_id: str
    payoff_rule_id: str
    description: str = ""
...
@dataclass(frozen=True)
class CouponSwapExchangePayoffRule(CouponDeterminationRule):
    """Declarative rule for coupon-exchange form of ratio-forward-like economics."""
    rule_id: str
    pair: FxPair
...
@dataclass(frozen=True)
class HitCashflowPolicyEffect(Effect):
    ...
    def validate(self) -> None:
        ...
        for component in self.components:
            if isinstance(component, CouponStreamLeg):
                ...
                _require(component.coupon_rule_id, rule_id_set, "coupon rule")
        ...
        for rule in self.determination_rules:
            if isinstance(rule, MtMResetQuantityRule):
                _require(rule.observation_rule_id, obs_rule_id_set, "observation rule")
            elif isinstance(rule, FloatingRateRule):
                ...
                _require(rule.condition_id, set(cond_ids), "condition")
            elif isinstance(rule, RateCouponRule):
                _require(rule.rate_rule_id, rule_id_set, "rate rule")
            elif isinstance(rule, StructuredCouponPayoffRule):
                _require(rule.payoff_rule_id, rule_id_set, "payoff rule")

        for accumulator in self.accumulators:
            for cid in accumulator.source_component_ids:
                _require(cid, component_id_set, "accumulator source component")

        condition_id_set = set(cond_ids)
        for lifecycle_rule in self.lifecycle_rules:
            if isinstance(lifecycle_rule, EventLifecycleRule):
                _require(lifecycle_rule.trigger.condition_id, condition_id_set, "trigger condition")
                ...
                _require(lifecycle_rule.target_condition_id, condition_id_set, "target condition")
...
class RuntimeStateV2:
    ...
    def initial_from_form(cls, form: ContractFormV2) -> "RuntimeStateV2":
        ...
        for component in form.components:
            if isinstance(component, (CouponStreamLeg, FundingLeg)):
                rule_id = component.notional_rule_id
                try:
                    rule = form.determination_rule_by_id(rule_id)
                    if isinstance(rule, FixedQuantityRule):
                        state.numeric_state[f"current_notional_{component.component_id}"] = decimal_value_on(rule.quantity)
                except KeyError:
                    pass
...
def evaluate_quantity_rule(form: ContractFormV2, state: RuntimeStateV2, rule_id: str, *, when: date | None = None, index: int | None = None, observation_value: Decimal | None = None) -> Decimal:
    rule = form.determination_rule_by_id(rule_id)
    if isinstance(rule, FixedQuantityRule):
...
def apply_observation(form: ContractFormV2, state: RuntimeStateV2, observation: ObservationRecord) -> None:
    ...
    for lifecycle_rule in form.lifecycle_rules:
        if isinstance(lifecycle_rule, EventLifecycleRule):
            ...
            quantity_rule = form.determination_rule_by_id(lifecycle_rule.quantity_rule_id)
            if not isinstance(quantity_rule, MtMResetQuantityRule):
                continue
            obs_rule = form.observation_rule_by_id(quantity_rule.observation_rule_id)
            if observation.observable_id != obs_rule.observable_id:
                continue
