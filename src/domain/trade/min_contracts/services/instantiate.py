...
from ..common.representations import CouponSwapRepresentation, OptionBundleRepresentation, TradeRepresentation
...
from ..workflow.trade import IndicationSnapshot, TradeDraft
...
def expand_schedule(schedule: ScheduleLike) -> EventSchedule:
    ...
    while current <= schedule.end_date:
        settlement_date = _add_months(current, 0)
...
def _coupon_cashflows_for_tarf(contract: TARFSpec, view_kind: str, schedule_id: str) -> list[CashflowRecord]:
    ...
    for event in schedule.events:
        ...
        if view_kind == "coupon_swap":
            ...
            out.append(CashflowRecord(
                ...
                original_amount_description=amount_desc,
                current_amount_description=amount_desc,
                original_payment_date=event.settlement_date.isoformat(),
                current_payment_date=event.settlement_date.isoformat(),
                metadata={"underlying": contract.underlying},
            ))
        else:
            call_desc = f"buy/sell call-style option slice derived from TARF event, K={strike}, qty ratio={ratio}"
            put_desc = f"buy/sell put-style option slice derived from TARF event, K={strike}, qty ratio={ratio}"
            out.append(CashflowRecord(
                ...
                currency=contract.settlement_currency,
                original_amount_description=call_desc,
                current_amount_description=call_desc,
                original_payment_date=event.settlement_date.isoformat(),
                current_payment_date=event.settlement_date.isoformat(),
            ))
            out.append(CashflowRecord(
                ...
                currency=contract.settlement_currency,
                original_amount_description=put_desc,
                current_amount_description=put_desc,
                original_payment_date=event.settlement_date.isoformat(),
                current_payment_date=event.settlement_date.isoformat(),
            ))
    return out
...
def _coupon_cashflows_for_coupon_swap(contract: CouponSwapSpec | AKOCouponSwapSpec, view_kind: str, schedule_id: str) -> list[CashflowRecord]:
...
def instantiate_trade_draft(
    *,
    ...
    if isinstance(contract, TARFSpec):
        ...
        cashflows = _coupon_cashflows_for_tarf(contract, representation.kind, schedule_id)
        ...
        cashflows = _coupon_cashflows_for_coupon_swap(contract, representation.kind, schedule_id)
    else:
